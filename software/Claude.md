...
- All register handlers (original, fast, ultra, cached ASM) now decode the three-word FIFO protocol generated by `board_registers.pio`; shared helpers in `dma.h` provide tag/address/data extraction and encoding.
- Core1 warm-up routines, debug logging, and the DOS `benchmark_irq` harness were updated to emit/consume tagged payloads, keeping the cached path exercised without injecting legacy words.
- New DOS helper `addr_mid.exe` (writes/reads 0x55 and 0xAA to DMA_ADDR_MID) passes reliably, confirming the mid-byte register works in isolation.
- The full `dmatest.exe` still reports mid-byte failures once it runs the broader address suite (0x55→0x00, 0xAA→0xA0, multi-byte reads returning stale values). Logs show repeated `FIFO_READ_COMMIT` traffic during the burst; suspected issue is state/cache interaction when the handler services consecutive commits without intervening writes.
- Deferred queue processing is seeing clean offsets (e.g. `0x7BCE0000` → DMA_ADDR_L, `0x7BCE8000` → DMA_ADDR_M); no bogus `0xEF300` prefetches observed after the decode refactor.
- Next steps: instrument the cached handler to verify commit-side updates for address bytes during extended bursts; consider temporarily switching to the non-cached handler during `dmatest` to compare results.
- Latest capture shows motherboard READY stretching during the failing cycles; logic analyzer spotted a single T-cycle where the PIO emits an extra FIFO entry, after which tag alignment stays off until a later compensating cycle—need to log raw tag sequences to pinpoint the stray push.
