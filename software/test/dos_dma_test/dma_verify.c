...
/* Set to 1 to use expansion RAM for DMA buffer instead of conventional memory */
/* Expansion RAM starts at 0x1FFFF (128KB) for 128KB motherboard */
/* Use 0x2FFFF to be safely in expansion territory */
/* Set to 0 to test with on-board RAM (isolates expansion RAM timing issues) */
#define USE_EXPANSION_RAM 1
#define EXPANSION_RAM_ADDR 0x2FFFF

/* Write test uses scratch sectors that won't corrupt the disk */
/* These are in the data area past the root directory */
...
typedef struct {
    uint16_t lba;
    uint8_t pattern_crc;     /* CRC of pattern we wrote */
    uint8_t readback_crc;    /* CRC of data read back */
    uint8_t passed;
    uint8_t phase_error;
    uint16_t write_polls;
    uint16_t read_polls;
} write_result_t;
 ...
 * With USE_EXPANSION_RAM, returns a fixed expansion RAM address instead.
 */
static uint32_t get_buffer_address(void) {
#if USE_EXPANSION_RAM
    /* Use fixed expansion RAM address for testing */
    return EXPANSION_RAM_ADDR;
#else
    uint16_t seg, off;
    uint32_t phys;
...
/*
 * Read a byte from the DMA buffer location.
 * When USE_EXPANSION_RAM is set, reads via far pointer from expansion RAM.
 */
static uint8_t read_dma_byte(uint16_t offset) {
#if USE_EXPANSION_RAM
    /* Convert physical address to segment:offset and read via far pointer */
    uint32_t phys = EXPANSION_RAM_ADDR + offset;
    uint16_t seg = (uint16_t)(phys >> 4);
    uint16_t off = (uint16_t)(phys & 0x0F);
    volatile uint8_t far *ptr = (volatile uint8_t far *)MK_FP(seg, off);
    return *ptr;
#else
    return dma_buffer[offset];
#endif
}

/*
 * Write a byte to the DMA buffer location.
 * When USE_EXPANSION_RAM is set, writes via far pointer to expansion RAM.
 */
static void write_dma_byte(uint16_t offset, uint8_t value) {
#if USE_EXPANSION_RAM
    /* Convert physical address to segment:offset and write via far pointer */
    uint32_t phys = EXPANSION_RAM_ADDR + offset;
    uint16_t seg = (uint16_t)(phys >> 4);
    uint16_t off = (uint16_t)(phys & 0x0F);
    volatile uint8_t far *ptr = (volatile uint8_t far *)MK_FP(seg, off);
    *ptr = value;
#else
    dma_buffer[offset] = value;
#endif
}

/*
 * Compute CRC of data in DMA buffer location.
 */
static uint8_t compute_dma_buffer_crc(void) {
    uint8_t crc = 0x00;
    uint16_t i;
    uint8_t j;
...
/*
 * Fill buffer with deterministic pattern based on LBA.
 * Pattern: byte[i] = (lba + i) & 0xFF
 * This allows both sides to independently compute expected CRC.
 * When USE_EXPANSION_RAM is set, writes to expansion RAM via far pointer.
 */
static void fill_test_pattern(uint32_t lba) {
    uint16_t i;
    for (i = 0; i < SECTOR_SIZE; i++) {
        uint8_t val = (uint8_t)((lba + i) & 0xFF);
        write_dma_byte(i, val);
#if !USE_EXPANSION_RAM
        /* Also keep dma_buffer in sync for CRC calculation */
        dma_buffer[i] = val;
#endif
    }
}
...
/*
 * Compute expected CRC for a given LBA test pattern.
 * This matches the pattern generated by fill_test_pattern().
 */
static uint8_t compute_pattern_crc(uint32_t lba) {
    uint8_t temp_buffer[SECTOR_SIZE];
    uint16_t i;
    for (i = 0; i < SECTOR_SIZE; i++) {
        temp_buffer[i] = (uint8_t)((lba + i) & 0xFF);
    }
    return crc8(temp_buffer, SECTOR_SIZE);
}

/*
 * Issue a SASI WRITE(6) command to write one sector.
 * Returns 0 on success, non-zero on error.
 */
static int write_sector(uint32_t lba, uint16_t *poll_count) {
    uint16_t polls;
    uint8_t status_byte;
    uint32_t dma_addr;
    ...
    *poll_count += polls;
    ...
    /* Step 10: Wait for status phase (DMA transfer happens automatically) */
    /* The Pico will DMA the sector data from Victor RAM, then enter status phase */
    polls = poll_status(PHASE_STATUS);
    if (polls == 0xFFFF) {
        return 9;  /* Timeout waiting for status (DMA may have failed) */
    }
...
/*
 * Reset the DMA board to recover from errors.
 */
...
static void run_test(void) {
    ...
    for (i = 0; i < NUM_TEST_SECTORS; i++) {
        ...
        /* Clear buffer before read (only for conventional memory) */
#if !USE_EXPANSION_RAM
        ...
        if (err != 0) {
            ...
            /* Reset board to recover from error state */
            reset_board();
            continue;
        }
        ...
        /* Copy first 8 bytes for verification (using read_dma_byte for expansion RAM support) */
        for (j = 0; j < 8; j++) {
            results[i].first8[j] = read_dma_byte(j);
        }

        /* Calculate CRC8 of received data */
        actual_crc = compute_dma_buffer_crc();
...
/*
 * Run the round-trip write test.
 * This tests: DOS fills buffer → DMA writes to disk → read back → verify
 * This exercises the dma_read_from_victor_ram() path on the Pico.
 */
static void run_write_test(void) {
    int i;
    int err;
    uint8_t pattern_crc;
    uint8_t readback_crc;

    printf("\nRound-trip write test (%d sectors)...\n", NUM_WRITE_TEST_SECTORS);
    printf("WARNING: This writes to LBAs 200-204. Ensure these are safe.\n\n");

    for (i = 0; i < NUM_WRITE_TEST_SECTORS; i++) {
        write_results[i].lba = write_test_lbas[i];

        /* Step 1: Fill buffer with known pattern */
        fill_test_pattern(write_test_lbas[i]);
        /* Use compute_pattern_crc for deterministic CRC (works with expansion RAM) */
        pattern_crc = compute_pattern_crc(write_test_lbas[i]);
        write_results[i].pattern_crc = pattern_crc;

        /* Step 2: Write the sector to disk via SASI */
        err = write_sector(write_test_lbas[i], &write_results[i].write_polls);
        if (err != 0) {
            write_results[i].phase_error = err;
            write_results[i].passed = 0;
            printf("  LBA %3u: WRITE PHASE ERROR %d\n",
                   write_test_lbas[i], err);
            /* Reset board to recover from error state */
            reset_board();
            continue;
        }

        /* Step 3: Clear buffer and read back the sector */
#if !USE_EXPANSION_RAM
        memset(dma_buffer, 0xAA, SECTOR_SIZE);
#endif
        err = read_sector(write_test_lbas[i], &write_results[i].read_polls);
        if (err != 0) {
            write_results[i].phase_error = 100 + err;  /* Offset to distinguish read errors */
            write_results[i].passed = 0;
            printf("  LBA %3u: READ PHASE ERROR %d\n",
                   write_test_lbas[i], err);
            continue;
        }

        /* Step 4: Verify read-back matches what we wrote */
        /* Use compute_dma_buffer_crc which reads from correct location (expansion/conventional RAM) */
        readback_crc = compute_dma_buffer_crc();
        write_results[i].readback_crc = readback_crc;
        write_results[i].phase_error = 0;

        if (readback_crc == pattern_crc) {
            write_results[i].passed = 1;
            printf("  LBA %3u: PASS (CRC=0x%02X, w_polls=%u, r_polls=%u)\n",
                   write_test_lbas[i], readback_crc,
                   write_results[i].write_polls, write_results[i].read_polls);
        } else {
            write_results[i].passed = 0;
            printf("  LBA %3u: FAIL pattern_crc=0x%02X readback_crc=0x%02X\n",
                   write_test_lbas[i], pattern_crc, readback_crc);
        }
    }
}
...
static void print_results(void) {
    ...
    printf("READ Tests: %d PASS, %d FAIL, %d ERROR\n",
           pass_count, fail_count, error_count);

    /* Write test summary */
    for (i = 0; i < NUM_WRITE_TEST_SECTORS; i++) {
        if (write_results[i].phase_error) {
            write_error++;
        } else if (write_results[i].passed) {
            write_pass++;
        } else {
            write_fail++;
        }
    ...
    printf("WRITE Tests: %d PASS, %d FAIL, %d ERROR\n\n",
           write_pass, write_fail, write_error);
    ...
    /* Write test failure details */
    if (write_fail > 0 || write_error > 0) {
        printf("Write test failure analysis:\n");
        printf("----------------------------\n");

        for (i = 0; i < NUM_WRITE_TEST_SECTORS; i++) {
            if (write_results[i].phase_error) {
                if (write_results[i].phase_error >= 100) {
                    printf("\nLBA %u: Read-back phase error %d\n",
                           write_results[i].lba, write_results[i].phase_error - 100);
                } else {
                    printf("\nLBA %u: Write phase error %d\n",
                           write_results[i].lba, write_results[i].phase_error);
                }
            } else if (!write_results[i].passed) {
                printf("\nLBA %u: Round-trip CRC mismatch\n", write_results[i].lba);
                printf("  Pattern CRC:   0x%02X\n", write_results[i].pattern_crc);
                printf("  Readback CRC:  0x%02X\n", write_results[i].readback_crc);
                printf("  This indicates DMA read-from-RAM or write-to-RAM failure\n");
            }
        }
        printf("\n");
    }
    ...
    /* Final verdict */
    if (pass_count == NUM_TEST_SECTORS && write_pass == NUM_WRITE_TEST_SECTORS) {
        printf("*** ALL TESTS PASSED ***\n");
        printf("DMA read and write transfers are working correctly.\n");
    } else {
        printf("*** TESTS FAILED ***\n");
        if (pass_count < NUM_TEST_SECTORS) {
            printf("Read tests failed: DMA write-to-RAM may be corrupting data.\n");
        }
        if (write_pass < NUM_WRITE_TEST_SECTORS) {
            printf("Write tests failed: DMA read-from-RAM may be corrupting data.\n");
        }
